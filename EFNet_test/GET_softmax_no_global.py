...
class EncoderLayer(nn.Module):
    ...
    def forward(self, bg, shape): # X(6144, 256)
        ...
        E = src + dst # 
        # 
        # E_mskd = E * bg.edata['edgmsk'] # 
        # bg.edata['aij'] = self.act(E_mskd) # 
        # bg.edata['rij'] = self.act(-E_mskd) # 
        # bg.update_all(fn.copy_e('aij', 'm'), fn.sum('m', 'A_sum')) # 
        # bg.update_all(fn.copy_e('rij', 'm'), fn.sum('m', 'R_sum')) # 
        # denominator = bg.ndata['A_sum'] + bg.ndata['R_sum'] + 1e-2 # 
        # bg.ndata['X'] = X #
        # X_skip = bg.ndata['X']
        ...
        bg.ndata['X'] = X # 
        X_skip = bg.ndata['X']
        bg.update_all(fn.u_mul_e('X', 'aij', 'm'), fn.sum('m', 'XA_sum')) # 
        X_trans = bg.ndata['XA_sum']  / (bg.ndata['A_sum'] + 1e-5) # 
        bg.ndata['X'] = X_skip + self.bn(X_trans) # -> (6144, 256)
        #
        X = bg.ndata['X'].permute(1,0).reshape(shape)  # 
...
class GETLayer_inter1(nn.Module):
    ...
    def forward(self, bg2): # 
        ...
        #
        bg2.update_all(fn.u_mul_e('Xe', 'aij', 'm'), fn.sum('m', 'XeA_sum')) # 
        Xe_trans = (bg2.ndata['XeA_sum'] + bg2.ndata['Xe']*bg2.ndata['R_sum']) / denominator #
        #
        return Xe_trans 
# =============================================================================
# =============================================================================
...
class GET_inter(nn.Module): #
    ...
    def forward(self, Xe, bg, shape): # 
        ...
        with self.Tg.local_scope():
            self.Tg.ndata['Xe'] = th.cat((Xe, SRC), dim=0) #
            nfeat = self.trans_encoder(self.Tg, num_dst) # 
        return nfeat
###################################################################
#
#
