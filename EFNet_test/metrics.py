# # -*- coding: utf-8 -*-
# """
# Created on Mon Oct 12 23:03:15 2020
#
...
# Source link:
# https://github.com/DIAGNijmegen/neural-odes-segmentation/blob/master/metrics.py#L7
...
def F1score(S=None, G=None):
    ...
    listS = np.unique(S)
    listS = np.delete(listS, np.where(listS == 0)) # 删除背景
    numS = len(listS)
    # print (numS)
    listG = np.unique(G)
...
def ObjectHausdorff(S=None, G=None):
    ...
    for iLabelS in range(len(listLabelS)):
        ...
        if intersectlist.any():
            indexGi = stats.mode(intersectlist).mode # 取众数
            Gi = (G == indexGi)
        else:
